...
--     for key, value in pairs(accuracy_settings) do
--         if value ~= 0 then
--             local signed_value = value > 0 and "+" .. tostring(value) or tostring(value)
--             local effect_str = "jar_adjustable_missiles__effect__acc__" .. key .. "__" .. signed_value
--             logger:write("Creating effect", effect_str)
--             effect_bundle:add_effect(effect_str, 'faction_to_force_own', 1)
--         end
--     end
    
--     for key, value in pairs(reload_settings) do
--         if value ~= 0 then
--             local signed_value = value > 0 and "+" .. tostring(value) or tostring(value)
--             local effect_str = "jar_adjustable_missiles__effect__rld__" .. key
--             logger:write("Creating effect", effect_str)
    ...
    for set, stats in pairs(settings) do
        for stat, value in pairs(stats) do
            ...
            elseif stat == 'reload' then
                local effect_str = "jar_adjustable_missiles__effect__rld__" .. set
                logger:write("effect_str", effect_str)
                effect_bundle:add_effect(effect_str, "faction_to_force_own", value)
            end
        end
