...
local config = core:get_static_object("adj_mis_config")
...
local function create_static_pages()
    ...
    local global_page = create_static_page("jar_unit_set_global", "Global")
    mct_mod:set_default_settings_page(global_page)
    -- Create the static Characters page.
    create_static_page("jar_unit_set_characters", "Characters")
end
...
local function create_categorical_bv_section(scope, section_title)
    local section = mct_mod:add_new_section("display__" .. scope, section_title)
...
local function create_dropdown_section()
    local section = mct_mod:add_new_section("display__dropdown", "Unit Category")
...
local function create_categorical_link_section()
    local section = mct_mod:add_new_section("display__link", "Link Player/AI Section")
    section:set_description("Toggle whether to share settings for Player and AI.")
...
local function create_categorical_actuals_page()
    ...
    for unit_set_key, unit_set_config in pairs(config.unit_set_config) do
        if not unit_set_config.static then
            local link_section = create_link_section(unit_set_key)
...
core:add_listener(
    ...
    function(context)
        ...
        -- Check that the option is for STAT
        local option_data = utils.get_option_data(option)
        local command = option_data.command

        if command ~= "STAT" or option_data.scope ~= "player" then return false end
        ...
        -- If all conditions pass (STAT, player, linked) return true
        return true
...
core:add_listener(
    ...
    function(context)
        ...
        if (command ~= "STAT" and command ~= "LINK") or option_data.unit_set_key ~= "display" then return false end

        -- If all conditions pass (STAT or LINK, categorical) return true
        return true
...
core:add_listener(
    ...
    function(context)
        ...
        for _, bonus_value_key in ipairs(bonus_value_keys) do
            ...
            local cat_ai_option = mct_mod:get_option_by_key(utils.create_bv_option_key("display", bonus_value_key, "ai"))
