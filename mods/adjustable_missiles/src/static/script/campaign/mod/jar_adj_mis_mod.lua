...
------------------------------------------------------------------------
--- Module: Adjustable Missiles Mod
--- Author: AceTheGreat
--- Description: Mod functionality for Adjustable Missiles
------------------------------------------------------------------------
...
local function load_mod_settings()
    ...
    for unit_set_key, _ in pairs(utils.get_active_unit_set_configs()) do
        ...
        for _, bonus_value_key in ipairs(utils.get_unit_set_bonus_value_keys(unit_set_key)) do
            ...
            if mct then
                local mod = mct:get_mod_by_key(mod_config.mod_name) -- NOTE: adj_com uses mod_prefix instead
